...
public class Util {
	...
	public static List<Line> drawAndReturnLine(BufferedImage imageBuffered, List<Point> points,String outUrl) throws Exception{
		...
		for(int i = 0; i < points.size(); i++) {
			...
			Line line = new Line();
			if(i < points.size() - 1) {
				line = Util.getLine(points.get(i), points.get(i + 1));
				linePoints= Util.getLineAllPoint(line, points.get(i), points.get(i + 1));
				
			}else {
				line = Util.getLine(points.get(0), points.get(i));
				linePoints= Util.getLineAllPoint(line, points.get(0), points.get(i));
			}
