...
public class Example {
	...
	public static void main(String[] args) throws Exception {
		...
		JSONObject res = client.detect(Util.getBase64Str(imageUrl), "BASE64", options);
	...
	private static void createResultImage(String path,JSONArray arrays) throws Exception{
		...
		for(int i = 0;i < arrays.length(); i++) {
			...
			Set<Point> lists = Util.getMarkCricle(new Point(x, y), 5);
			int mark = 0x0000ff;
	...
	private static void getLeftEye(String path,JSONArray landmarkEye) throws Exception {
		...
		List<Line> lines = Util.drawAndReturnLine(imageBuffered, points, outUrl);
	}
